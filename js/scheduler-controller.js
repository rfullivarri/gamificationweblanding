...
import {
  ...
  apiMarkFirstProgrammed               // ⬅️ NUEVO
...
async function ensureCtx() {
  ...
  const ctx = await apiGetContext(email);
  saveCtx(ctx);
  return ctx;
}
...
export function attachSchedulerModal() {
  ...
  modal.addEventListener('schedule:save', async (ev) => {
    ...
    try {
      ...
      // 2) Resolver sheetId (con fallback)
      let sheetId =
        ctx.userSheetId ||
      ...
      // 5) Marcar L='SI' en HUB (no bloqueante)
      try {
      ...
      // 7) Refrescar bundle y re-pintar
      try {
